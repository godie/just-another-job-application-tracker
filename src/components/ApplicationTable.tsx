    ...
    <>
      ...
      <div className='md:hidden space-y-3' data-testid='application-cards'>
        {paginatedData.length === 0 ? (
          <div className='bg-muted p-8 border border-border text-center text-muted-foreground italic text-sm font-medium'>
      ...
      <Card className='hidden md:block overflow-hidden border border-border'>
        <Table data-testid='application-table'>
          <TableHeader>
            <TableRow className='hover:bg-transparent'>
              ...
              {columns.map((column) => (
                <TableHead
                  key={column.id}
                  className='px-4 sm:px-6 py-3 text-[11px] sm:text-xs font-semibold text-foreground uppercase tracking-wide bg-muted whitespace-nowrap'
                >
                  {column.label}
                </TableHead>
              ))}
              <TableHead className='relative px-4 sm:px-6 py-3 w-1 bg-muted'>
          ...
          <TableBody>
            {paginatedData.length === 0 ? (
              <TableRow>
                <TableCell colSpan={columns.length + (hasPositionColumn ? 1 : 2)} className='px-4 sm:px-6 py-10 text-center text-muted-foreground italic text-sm font-medium' role='status' aria-live='polite'>
                  {t('home.noApplications')}
                </TableCell>
              </TableRow>
            ) : (
      ...
      {totalPages > 1 && (
        <div className='flex items-center justify-center gap-2 pt-4'>
          <button
            ...
            className='px-4 py-2 text-sm font-medium text-foreground bg-card border border-border rounded hover:bg-muted disabled:opacity-50 disabled:cursor-not-allowed transition'
