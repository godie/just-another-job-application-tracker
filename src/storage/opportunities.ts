...
export const getOpportunities = (): JobOpportunity[] => {
  try {
    ...
    // ⚡ Bolt: Centralize sanitization at the storage layer.
    return opportunities.map(opp => sanitizeObject(opp as unknown as Record<string, unknown>)) as JobOpportunity[];
  } catch (error) {
