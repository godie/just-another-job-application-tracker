...
const LandingNav: React.FC<{ onNavigate: (page: PageType) => void }> = ({ onNavigate }) => {
  const { t, i18n } = useTranslation();
...
const LandingHero: React.FC<{ onNavigate: (page: PageType) => void }> = ({ onNavigate }) => {
...
const LandingCTA: React.FC<{ onNavigate: (page: PageType) => void }> = ({ onNavigate }) => {
  const { t } = useTranslation();
  const handleEnterApp = () => {
    setSkipAuthModal();
    onNavigate('applications');
