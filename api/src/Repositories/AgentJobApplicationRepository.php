...
class AgentJobApplicationRepository
{
    ...
    public function findById(int $id): ?AgentJobApplication
    {
        $stmt = $this->db->prepare('SELECT * FROM agent_job_applications WHERE id = :id LIMIT 1');
        $stmt->execute(['id' => $id]);
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        return $row ? AgentJobApplication::fromDatabase($row) : null;
    }

    public function findByHash(string $hash): ?AgentJobApplication
    {
        $stmt = $this->db->prepare('SELECT * FROM agent_job_applications WHERE idempotency_hash = :hash LIMIT 1');
    ...
    public function list(array $filters = []): array
    {
        ...
        $sortColumn = in_array($filters['sort_by'] ?? '', ['applied_at', 'created_at', 'company_name', 'job_title'], true)
            ? $filters['sort_by']
            : 'created_at';
