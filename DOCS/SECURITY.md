...
- **Mitigation:** We have implemented a two-pronged approach to prevent XSS:
    - **Input Sanitization:** All user data is sanitized on the frontend using the `dompurify` library before it is stored (via Zustand stores backed by localStorage) or rendered in the application. This is done in `src/utils/localStorage.ts` and `src/components/ApplicationTable.tsx`.
...
- **Mitigation:** All user-provided data is sanitized on the backend using `htmlspecialchars` before it is processed or sent to the Google Sheets API. This is done in the `GoogleSheetsController`. We also validate the format of the `spreadsheetId` to ensure it is a valid Google Sheets ID.
...
- **Mitigation:** We have implemented a strict CORS policy on the backend (via `OverPHP\Helpers\corsSendHeaders`) that only allows requests from the application's frontend origins. This is configured centrally in `api/index.php` and `api/config.php`.
