...
- [DOCS/FOLLOWUP_MATCHING_REFACTOR.md](./FOLLOWUP_MATCHING_REFACTOR.md) — `src/utils/matching.ts` deferred refactor. Trigger: matching.ts > ~700 LOC OR a **5th** `js-set-map-lookups` suppression is added (4 already concentrated here per audit memo). Two candidates in yield-vs-blast-radius order (extract module-scope constants to `src/utils/matching/constants.ts` first; extract scoring sub-module to `src/utils/matching/scoring.ts` second). **Audit-anchor 141 LOC at `cdafe81` vs current HEAD `5310761` working tree 446 LOC = 3.15× drift noted in the tracker.**
