...
> Status: **partly implemented as of 2.8.0**. Opportunity match scoring (§1), the email →
> application match (§2) and the email event classification (§3) run through
...
> Still open: field extraction (§3, the company/position spans), CSV header mapping (§5),
> work-type and date normalisation (§6). Before extending the integration, read
