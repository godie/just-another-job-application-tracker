...
### Added
- **`scripts/check-orphans.sh` `sequence` subcommand** — walks `CHANGELOG.md`'s `## [X.Y.Z] - YYYY-MM-DD` heading sequence and flags every non-contiguous PATCH gap within each `(major, minor)` group. For each minor `M.N` present in the changelog, the script groups the PATCH values, walks them in ascending order, and emits one row per missing patch in `(prev_patch, cur_patch)` when `cur_patch - prev_patch > 1`. Output is a 5-column Markdown table `| Missing | Between | Between date | And | And date |` so a contributor seeing the tracking issue for the first time has the dates of the surrounding shipped versions and can decide whether to bridge the gap with a one-liner placeholder heading. Skips `[Unreleased]` (no semver), preserves parenthetical annotations like `(later)`, dedupes duplicate headings (`sort -u`), and does not flag major or MINOR transitions as gaps (advancing `2.5.x → 2.6.0` is a deliberate MINOR bump, not a lost slot). The script is dispatched by subcommand: `scripts/check-orphans.sh literal <version>` for the existing rg-based orphan sweep, `scripts/check-orphans.sh sequence` for the gap check. Aliases `orphans` / `orphan` (literal) and `gaps` / `gap` (sequence) are accepted by the dispatcher and documented in `--help`. Backward compat: a bare semver-shaped positional arg falls through to `literal <version>` against the strict pattern `^[0-9]+\.[0-9]+\.[0-9]+$` (no pre-release suffix accepted; that relaxation is YAGNI for this repo where no `package.json` has ever carried one, and reintroducing it later is a one-line edit). Help text and implementation notes are kept in lock-step with the dispatch (single `case` statement). The 2.5 → 2.6 boundary that ends at `2.6.2` (current main HEAD) fires this check as one tracking issue today — three missing patches in `(2.6.x)` until those slots are claimed or bridged.
- **`.github/workflows/version-sequence-check.yml`** — daily + per-release audit for the new gap check. Trigger matrix mirrors `orphan-sweep.yml` (`release.published`, daily at 06:00 UTC cron, `workflow_dispatch`) so the two audits share operational cadence. Single sticky tracking issue labeled `version-sequence-check` (idempotent upsert; closes-with-comment on clean). Permissions: `contents: read` + `issues: write`. Concurrency-cancellable so a burst of release events does not pile up redundant runs. Tracking-issue body explains the placeholder-heading remediation: a real entry (even a one-liner with no body) bridges the gap so the script goes green, preserving the forensic trail of skipped slots without forcing a re-bump.
