...
### Security
- Hardened Google ID-token handling in `api/src/Controllers/AppAuthController.php` by requiring a well-formed RS256 JWT with a non-empty signature, rejecting `none` and attacker-controlled `jwk`/`jku`/`x5u` headers, and validating Google's issuer, audience, expiration, subject, and verified-email claims after Google's remote signature verification.

### Fixed
- Synced the `LogfireTelemetry` `SERVICE_VERSION` / `INSTRUMENTATION_VERSION` constants from `2.7.0` to `2.7.3` (missed by the 2.7.1 / 2.7.2 version bumps).
...
### Dependencies
- Pinned transitive `qs` to `6.16.0` and `fast-uri` to `3.1.7` through npm overrides to clear the NPM Audit and CVE Scan findings without upgrading their parent packages.
...
- **Same per-entity Zod validation on the Networking write path** (`src/storage/networking.ts`) — `saveNetworkingWorkspace` now runs every list through the same parsers before serializing, so invalid rows never reach `localStorage` (assertions inspect the raw persisted payload, not the read filter). The shared `fromWorkspace` helper makes read/write symmetric: `getNetworkingWorkspace(saveNetworkingWorkspace(x)) ≡ x` whenever `x` is well-formed. Storage stays a defensive boundary for store mutators, content-script bypass writes, future migration scripts, and cloud-sync merges.
- **`src/storage/networking.write.test.ts`** — 9 new assertions for the write path: round-trip symmetry, per-entity-class drop semantics (contact, interaction, follow-up, link, referral), `schemaVersion` rejection, single `jobNetworkingUpdated` event after a drop-laden save, and a sanity check on the raw persisted payload.
...
- **Networking contacts and follow-up workspace UI** (`src/pages/NetworkingPage.tsx`, `src/components/networking/ContactForm.tsx`, `ContactList.tsx`, `InteractionForm.tsx`, `FollowUpList.tsx`) — accessible candidate-side dashboard reachable via the new `networking` route (`PageType`, lazy-chunked, picked up by `Sidebar` and `BottomNav`). Empty state with localized "Add contact" CTA; per-contact panels surfacing interactions, application/opportunity links, and referrals; follow-up tasks partitioned into Overdue / Today / Upcoming by local calendar date with "Mark complete" inline actions. All forms use labelled inputs (Radix Dialog with associated titles, semantic `<fieldset>` + `<legend>` for radio groups, no color-only state). No bidirectionality assumed: applications and opportunities are read-only selectors from their existing stores.
- **Store extensions for links, referrals, and interactions** (`src/stores/networkingStore.ts`) — `addContactLink`, `addReferral`, `addInteraction` mirror the existing `addContact` / `addFollowUpTask` shape. Links and referrals enforce `(contactId, resourceType, resourceId)` uniqueness in the store and return `false` on duplicate so the UI can surface a translated alert without bypassing the dedup invariant. `getLinksForContact` / `getReferralsForContact` scope views to a single contact for cheap re-renders.
- **Networking i18n namespace + `nav.networking`** (`src/locales/{en,es}/translation.json`) — translated page title, intro, empty state, sections, contact/interaction/follow-up labels, link/referral dialogs, and an actionable duplicate-link warning. Test assertions reference translated strings (not raw keys), so the page renders correctly in both locales out of the box.
- **`src/pages/NetworkingPage.test.tsx`** — 8 new assertions covering the empty state, labelled contact creation, Overdue-before-Today-before-Upcoming ordering (with `vi.useFakeTimers()` + `vi.setSystemTime()` for the calendar-date boundary), interaction logging under a known contact, link add + duplicate alert, referral add, and complete-from-contact-row. `src/stores/networkingStore.links.test.ts` adds 9 store-level assertions (link add + persistence, duplicate rejection, two-resource variant, referral default/explicit status, link vs referral independence, by-contact scoping, raw persisted-payload sanity). `src/components/Sidebar.test.tsx` was bumped from 7 to 8 nav buttons to track the route addition.
- **Cloud schema and owner-scoped CRM API** — Task 3 of the Networking CRM plan ships the persistence + endpoint layer behind the local workspace. `db/migrations/20260729090000_CreateNetworkingCrmTables.php` creates five owner-scoped tables (`network_contacts`, `network_interactions`, `network_follow_up_tasks`, `network_contact_links`, `network_referrals`) with FKs to `users(id)` (RESTRICT) and on-cascade deletes on `contact_id`. Every row carries `owner_user_id`; every DELETE/INSERT prepared statement is parameterized so calls cannot accidentally widen scope. `api/src/Repositories/NetworkingRepository.php` exposes `replaceWorkspace(int $ownerUserId, array $workspace): void` (single PDO transaction that wipes the owner's prior rows in dependency order before re-inserting) and `getWorkspace(int $ownerUserId): array` (Task 1 shape, with `owner_user_id` and `tags_json` stripped on read). `api/src/Controllers/NetworkingController.php` returns the new `/sync/networking` endpoints with auth from the session (401), 400 for malformed JSON, and 422 for any of: missing top-level keys, unsupported enum values for `relationship_type`/`channel`/`status`/`resource_type`/`referral.status`, or malformed ISO dates. Body never carries `owner_user_id` — the controller derives it from the session, mirroring the existing `/sync/applications` contract.
- **Wired networking sync routes** (`api/index.php`) — `GET /sync/networking` and `POST /sync/networking` closures guarded by `RequireAuth::handle()` like the existing `/sync/applications` endpoints. POST verb (not PUT) for symmetry with the rest of the sync family.
- **Schema documentation** (`docs/DB_SCHEMA.md`) — appends a `Networking CRM Tables` section covering all five new tables, their index strategy (`(owner_user_id)`, `(owner_user_id, due_at)` to power the day-partitioning on the dashboard, `(owner_user_id, contact_id, resource_type, resource_id)` for the link/referral dedup invariant), and the new endpoints.
- **`api/tests/Repositories/NetworkingRepositoryTest.php`** (7) and **`api/tests/Controllers/NetworkingControllerTest.php`** (12) — owner-isolation invariants tested at the SQL boundary (cross-owner rows are never read or written by the wrong user), transaction atomicity (FK violation during a child insert rolls back the whole replace), and every 401 / 422 / 200 path on the controller. SQLite in-memory fixtures mirror the Phinx schema so the SQL exercised here matches what production MySQL will receive. PHPStan clean (level 6) on both new files.

### Fixed
- **Code-review follow-ups on the Networking CRM** (4 Important findings from the branch review, all closed under this version per the one-branch-one-version rule):
  - **Reactive-sync listener pair on `NetworkingPage`** (`src/pages/NetworkingPage.tsx`) — the page now subscribes to the standard `storage` (keyed on `jobNetworking`, with `e.key === null` covering `storage.clear()`) + same-tab `jobNetworkingUpdated` custom event listener pair, so cross-tab and bypass writes refresh an already-open Networking tab instead of requiring a remount. Mirrors the `OpportunitiesPage` pattern per the AGENTS.md reactive-state-sync table.
  - **Networking wired into `useCloudSync`** (`src/hooks/useCloudSync.ts`) — push: third POST leg to `/api/sync/networking` with the workspace snapshot, subscribed via the `jobNetworkingUpdated` event in the same 2s-debounce effect; pull: `GET /api/sync/networking` response validated by the new `src/utils/networkingSyncSchemas.ts` (reusing the local per-entity parsers) and applied via the new store `setWorkspace`, replacing local data only from a valid envelope with at least one row so an empty/invalid envelope never wipes local data. Also fixes a real ordering bug the new tests exposed: the push effect previously gated on `_initialLoadDone` at effect-run time, but that flag flips asynchronously after the first pull completes and the effect never re-ran — push listeners never subscribed after a normal login. Subscription is now unconditional with the guard checked at fire time inside the timer callback.
  - **Store write-path Zod validation with user-facing errors** (`src/stores/networkingStore.ts`, `src/utils/networkingSchemas.ts`, `src/components/networking/ContactList.tsx`) — `addContact` / `addInteraction` / `addFollowUpTask` now return an error message (and `addContactLink` / `addReferral` return `false`) when the assembled entity fails the shared per-entity Zod schema, so a malformed row (empty name from an empty form submit, over-long notes) is rejected at the UI boundary instead of being persisted and silently dropped on the next read. `ContactList` surfaces rejections through `useAlert` (`showError`) with the translated `networking.contacts.invalid` prefix.
  - **New `src/utils/networkingSyncSchemas.ts`** — Zod validation for the `GET /api/sync/networking` response envelope, reusing the local per-entity parsers so the cloud is never more permissive than local storage; dropped-row counts surface as warnings.

### Validation
- 978 Vitest tests pass (6 new: 2 listener-pair regressions on `NetworkingPage`, 4 `useCloudSync` tests for the networking push/pull legs including the pull-no-wipe invariant and the push-subscription ordering bug). `npx tsc --noEmit`, `npm run lint`, and `npm run build` all pass. PHP API suite unchanged at 92 tests / PHPStan clean.
